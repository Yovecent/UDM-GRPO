...
class Trainer(object):
    ...
    def __init__(self, config, accelerator, logger):
        ...
        if self.ema and config.experiment.resume_iter > 0:
            ...
            if not os.path.exists(ema_weights):
                ema_weights = os.path.join(ema_ckpt, config.model.name, "adapter_model.bin")
    ...
    def save(self):
        ...
        if self.accelerator.is_main_process and not os.path.exists(f):
            ...
            if self.ema is not None and os.path.exists(os.path.join(f, "config.json")):
            ...
            elif self.ema is not None and os.path.exists(os.path.join(f, "adapter_config.json")):
                config_json = os.path.join(f, "adapter_config.json")
                f = f.replace("checkpoints", "ema_checkpoints")
                os.makedirs(f), shutil.copy(config_json, os.path.join(f, "adapter_config.json"))
                f = os.path.join(f, "adapter_model.bin")
                torch.save(self.ema.model.state_dict(), f)
