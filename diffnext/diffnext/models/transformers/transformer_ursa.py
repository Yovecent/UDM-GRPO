    ...
    def forward(
        ...
        **kwargs,
        ...
        def flash_loss(logits, labels):
            logits, labels = logits.flatten(0, 1), labels.flatten()
            loss_args = {"label_smoothing": inputs.get("label_smoothing", 0)}
            if cross_entropy_loss:
                return cross_entropy_loss(logits, labels, inplace_backward=True, **loss_args)[0]
            return nn.functional.cross_entropy(logits, labels, reduction="none", **loss_args)

        if labels is not None and isinstance(labels, torch.Tensor):  # NTP loss.
            lbls = torch.nn.functional.pad(labels[:, 1:], (0, 1), value=-100)
            loss = flash_loss(logits.float(), lbls).view(lbls.shape)
