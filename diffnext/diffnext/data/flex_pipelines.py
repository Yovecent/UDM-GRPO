...
class PromptPipe(object):
    """Pipeline to transform prompt features."""
    ...
    def get_outputs(self, inputs):
        """Return the outputs."""
        (label, caption), outputs = self.parse_annotations(inputs), {}
        outputs.setdefault("prompt", [label]) if label is not None else None
        outputs.setdefault("prompt", [caption]) if caption is not None else None
        outputs.setdefault("motion", [inputs["flow"]]) if "flow" in inputs else None
        outputs.setdefault("metadata", [inputs.get("metadata", "")])
        return outputs
...
class PromptWorker(PromptPipe, Worker):
    """Worker to transform prompt features."""
