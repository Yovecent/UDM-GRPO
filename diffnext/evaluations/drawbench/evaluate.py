...
from diffnext.rewards.reward_image import ComposeScorer
...
if __name__ == "__main__":
    ...
    scorers = ["pickscore", "clipscore", "hpsv2"]
    model = ComposeScorer(scorers, batch_size=args.batch_size).to(device)
    data_iter = list(map(model.scorers[0].batch_iter, (all_images, all_prompts)))
    ...
    for images, prompts in tqdm.tqdm(zip(*data_iter), total=len(data_iter[0])):
        ...
        for name, scorer in zip(scorers, model.scorers):
            results[name] += scorer(images, prompts)[1]
    for name, result in results.items():
        print(f"{name} =", sum(result) / len(result))
