...
# ------------------------------------------------------------------------
"""PickScore sampling for URSA models."""
...
def parse_args():
    ...
    parser.add_argument("--sample_size", type=int, default=1, help="sample size for each prompt")
...
if __name__ == "__main__":
    ...
    for step in tqdm(range(0, len(prompt_list), args.prompt_size), disable=rank):
        ...
        for i, img in enumerate(samples):
            ...
            meta_data = {"prompt": prompts[i % len(prompts)]}
            json.dump(meta_data, open(f"{prompt_root}/metadata.jsonl", "w"))
